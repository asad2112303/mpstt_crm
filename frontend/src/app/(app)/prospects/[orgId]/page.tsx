...
import {
  ...
  type Activity, type Branch, type Contact, type Organization,
  type ProspectStage, type SampleRow, type Task, type PriceRow,
} from "@/lib/types/crm";
...
function ActivitiesTab({ orgId }: { orgId: string }) {
  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery({
...
function SamplesTab({ orgId }: { orgId: string }) {
  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery({
...
function PricesTab({ orgId }: { orgId: string }) {
  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery({
  ...
  return (
    <main className="space-y-6 p-6">
      ...
      <PageHeader
        ...
        actions={org.prospect_profile ? <StageBadge stage={org.prospect_profile.stage} /> : undefined}
      />
